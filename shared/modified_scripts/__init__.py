...
def add_new_ramp_input(nml_doc, cell, delay, duration, startAmplitude, finishAmplitude, baselineAmplitude, params):
    id = get_next_stim_id(nml_doc,cell)
    
    input = RampGenerator(id=id, baselineAmplitude=baselineAmplitude, delay=delay, duration=duration, finishAmplitude=finishAmplitude, startAmplitude=startAmplitude)
    nml_doc.ramp_generators.append(input)
